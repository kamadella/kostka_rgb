...
namespace kostka_rgb.Models
{
    ...
    public static class ColorFormula
    {
        ...
        public static BitmapSource GenerateHSVGradient()
        {
            // Generates a Bitmap using the color callbacks.
            var bitmap = CreateHSVTexture();
            // Converts the Bitmap to a BitmapSource for use with WPF.
            var bitmapSource = Imaging.CreateBitmapSourceFromHBitmap(
                bitmap.GetHbitmap(),
                IntPtr.Zero,
        ...
        public static Bitmap CreateHSVTexture()
        {
            int width = 512;
            int height = 512;
            ...
            bmp.UnlockBits(data);
            SaveGradient(bmp, "HSV_Gradient.png"); // Save the gradient for debugging
            return bmp;
        }
